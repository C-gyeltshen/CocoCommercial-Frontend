...
import { CalendarDays, MapPin, Phone, User, Calendar, Component } from "lucide-react";
...
const OrderDetailsPopup: React.FC<OrderDetailsPopupProps> = ({ order, children }) => {
  ...
  if (!order) {
    return <p>Loading order details...</p>; // Fallback UI when `order` is undefined
  }
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="w-[95vw] max-w-[425px] p-4 sm:p-6 overflow-y-auto max-h-[90vh]">
        <DialogHeader>
          <DialogTitle className="text-lg sm:text-xl font-serif mb-4">
            Order Details #{order?.id ?? "N/A"}
          </DialogTitle>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4">
              <div className="flex items-center space-x-3 text-gray-600">
                ...
                <span className="text-sm sm:text-base break-words">{order?.customerName ?? "N/A"}</span>
              </div>
              <div className="flex items-center space-x-3 text-gray-600">
                <Phone className="h-5 w-5 flex-shrink-0" />
                <span className="text-sm sm:text-base">{order?.phoneNumber ?? "N/A"}</span>
              </div>
              <div className="flex items-center space-x-3 text-gray-600">
                <MapPin className="h-5 w-5 flex-shrink-0" />
                <span className="text-sm sm:text-base break-words">{order?.location ?? "N/A"}</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-2 text-gray-600 text-sm sm:text-base">
              <p>
                ...
                <span className="break-words ml-2">{order?.address ?? "N/A"}</span>
              </p>
              <p>
                <span className="font-medium">Delivery Notes:</span> 
                <span className="break-words ml-2">{order?.deliveryNotes ?? "N/A"}</span>
              </p>
              <p>
                ...
                order.items.map((item, index) => (
                  <div key={index} className="flex flex-col sm:flex-row justify-between items-start sm:items-center py-2 border-b">
                    <div className="flex items-center space-x-3 mb-2 sm:mb-0">
                      <img src={item.image} alt={item.name} className="h-10 w-10 rounded-lg flex-shrink-0" />
                      <div>
          ...
          <div className="border-t pt-4">
            <div className="flex justify-between items-center">
              ...
              <span className="font-medium text-base sm:text-lg">
                Nu.{order?.items?.reduce((sum, item) => sum + item.total, 0).toLocaleString() ?? "0"}
              </span>
            </div>
          </div>
        </div>
